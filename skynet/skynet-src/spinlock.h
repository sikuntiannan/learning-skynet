...
#define atomic_flag_test_and_set_ atomic_flag_test_and_set //设置真，并且返回之前的值。
#define atomic_flag_clear_ atomic_flag_clear//设置假。
...
spinlock_lock(struct spinlock *lock) {
	while (atomic_flag_test_and_set_(&lock->lock)) {}//一个自旋锁
}

static inline int
spinlock_trylock(struct spinlock *lock) {
	return atomic_flag_test_and_set_(&lock->lock) == 0;//只试一次，返回是否成功加锁。
}

static inline void
spinlock_unlock(struct spinlock *lock) {
	atomic_flag_clear_(&lock->lock);//设置为假。
}
